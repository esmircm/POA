...
class Tblmunicipio extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('clvestado, clvmunicipio_sunacoop, clvusuario, gid', 'numerical', 'integerOnly'=>true),
			array('strid_estado', 'length', 'max'=>2),
			array('strid', 'length', 'max'=>5),
			array('strdescripcion', 'length', 'max'=>40),
			array('blncapital, blnborrado, dtmmodificado, geometrico', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('clvcodigo, clvestado, strid_estado, strid, clvmunicipio_sunacoop, strdescripcion, blncapital, blnborrado, dtmregistro, dtmmodificado, clvusuario, gid, geometrico', 'safe', 'on'=>'search'),
		);
	}
