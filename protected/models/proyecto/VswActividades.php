...
class VswActividades extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_actividades, cantidad, fk_unidad_medida, fk_accion, fk_proyecto', 'numerical', 'integerOnly'=>true),
			array('actividad', 'length', 'max'=>200),
			array('unidad_medida', 'length', 'max'=>1000),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_actividades, actividad, cantidad, fk_unidad_medida, unidad_medida, fk_accion, fk_proyecto', 'safe', 'on'=>'search'),
		);
	}
