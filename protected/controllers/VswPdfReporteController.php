...
class VswPdfReporteController extends Controller
{
/**
...
{
$model=new VswPdfReporte;

// Uncomment the following line if AJAX validation is needed
// $this->performAjaxValidation($model);

if(isset($_POST['VswPdfReporte']))
{
$model->attributes=$_POST['VswPdfReporte'];
if($model->save())
...
'model'=>$model,
    ...
    public function actionCertificado() {
        $sql = 'select sec.* from evaluacion.vsw_pdf_reporte sec limit 611';
        ...
        while ($i < $val) {
            ...
            $html.='
            <tr class="tg">
                <td class="tg" width="1%">&nbsp;' . $row[$i]["cedula_evaluado"] . '</td>
                <td class="tg" width="1%">&nbsp;' . $row[$i]["nombres_evaluado"] . '</td>
                <td class="tg" width="1%">&nbsp;' . $row[$i]["apellidos_evaluado"] . '</td>
                <td class="tg" width="1%">&nbsp;' . $row[$i]["cargo_evaluado"] . '</td>
                <td class="tg" width="1%">&nbsp;' . $row[$i]["oficina_evaluado"] . '</td>
                <td class="tg" width="2%">&nbsp;' . $row[$i]["periodo_evaluacion"] . '</td>
                <td class="tg" width="2%">&nbsp;' . $row[$i]["cant_objetivos"] . '</td>
                <td class="tg" width="2%">&nbsp;' . $row[$i]["peso_total"] . '</td>
                <td class="tg" width="5%">&nbsp;' . $row[$i]["estatus"] . '</td> 
            ';
            $html.='</tr>';
            $i++;
        }
...
{
$dataProvider=new CActiveDataProvider('VswPdfReporte');
...
{
$model=new VswPdfReporte('search');
$model->unsetAttributes();  // clear any default values
if(isset($_GET['VswPdfReporte']))
$model->attributes=$_GET['VswPdfReporte'];
...
{
$model=VswPdfReporte::model()->findByPk($id);
if($model===null)
...
{
if(isset($_POST['ajax']) && $_POST['ajax']==='vsw-pdf-reporte-form')
{
