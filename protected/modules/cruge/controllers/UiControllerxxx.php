...
class UiController extends Controller {
    ...
    public function actionPwdRec() {
        ...
        if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeLogon']])) {
            ...
            if ($model->validate()) {
                ...
                Nucleo::envio_Usuario_generacion_clave($model->getModel(), $newPwd);
    ...
    public function actionUserManagementUpdate($id) {
        $this->_editUserProfile(Yii::app()->user->um->loadUserById($id), true, $id);
    }

    public function _editUserProfile(ICrugeStoredUser $model, $boolIsUserManagement, $id) {
        $this->layout = '///layouts/admintui';
        $userdetail = new CrugeUserdetail();
            ...
            if ($model->validate()) {
                ...
                if (Yii::app()->user->um->save($model, 'update')) {
                    if ($boolIsUserManagement == true) {
                        $this->redirect(array('DesactivarUsuarios'));
                    } else {
                        //$this->redirect(array('usersaved', 'layout' => $this->layout));
                        $this->redirect(array('/cruge/ui/editprofile'));
                    }
                }
            }
        }
        if (isset($_POST['CrugeUserdetail']['institution'])) {
            $idUserDetail = CrugeUserdetail::byIdUSerdetail($id);
            if (isset($idUserDetail) && !empty($idUserDetail)) {
                CrugeUserdetail::model()->updateByPk($idUserDetail->iduserdetail, array('institution' => $_POST['CrugeUserdetail']['institution']));

                if (isset($_POST['CrugeUserdetail']['businessoffice'])) {

                    CrugeUserdetail::model()->updateByPk($idUserDetail->iduserdetail, array('businessoffice' => $_POST['CrugeUserdetail']['businessoffice']));
                }
                if (isset($_POST['CrugeUserdetail']['coordination'])) {
                    CrugeUserdetail::model()->updateByPk($idUserDetail->iduserdetail, array('coordination' => $_POST['CrugeUserdetail']['coordination']));
                }
            } else if (!isset($idUserDetail)) {
                $command = Yii::app()->db->createCommand();
                if (isset($_POST['CrugeUserdetail']['institution']) && isset($_POST['CrugeUserdetail']['businessoffice']) && isset($_POST['CrugeUserdetail']['coordination']) && !empty($_POST['CrugeUserdetail']['institution']) && !empty($_POST['CrugeUserdetail']['businessoffice']) && !empty($_POST['CrugeUserdetail']['coordination']) && !empty($_POST['CrugeUserdetail']['appointment'])) {
//                    var_dump('aqui');die;
                    $command->insert('acceso.cruge_userdetail', array(
                        'iduser' => $id,
                        'institution' => $_POST['CrugeUserdetail']['institution'],
                        'businessoffice' => $_POST['CrugeUserdetail']['businessoffice'],
                        'coordination' => $_POST['CrugeUserdetail']['coordination'],
                        'appointment' => $_POST['CrugeUserdetail']['appointment'],
                    ));
                } else if (isset($_POST['CrugeUserdetail']['institution']) && !empty($_POST['CrugeUserdetail']['businessoffice']) && !empty($_POST['CrugeUserdetail']['coordination']) && empty($_POST['CrugeUserdetail']['appointment'])) {
                    $command->insert('acceso.cruge_userdetail', array(
                        'iduser' => $id,
                        'institution' => $_POST['CrugeUserdetail']['institution'],
                        'businessoffice' => $_POST['CrugeUserdetail']['businessoffice'],
                ...
                } else if (isset($_POST['CrugeUserdetail']['institution']) && !empty($_POST['CrugeUserdetail']['businessoffice']) && empty($_POST['coordinacion']) && !mpty($_POST['CrugeUserdetail']['appointment'])) {
                    $command->insert('acceso.cruge_userdetail', array(
                        'iduser' => $id,
                        'institution' => $_POST['CrugeUserdetail']['institution'],
                        'businessoffice' => $_POST['CrugeUserdetail']['businessoffice'],
                    ));
                } else if (isset($_POST['CrugeUserdetail']['institution']) && empty($_POST['CrugeUserdetail']['businessoffice']) && empty($_POST['coordinacion']) && empty($_POST['CrugeUserdetail']['appointment'])) {
                    $command->insert('acceso.cruge_userdetail', array(
                        'iduser' => $id,
                        'institution' => $_POST['CrugeUserdetail']['institution'],
                    ));
    ...
    public function actionUserManagementCreate() {
        ...
        $list = CrugeMain::datalist();

        if (isset($_POST['cedula'])) {

            $sql = "select value from acceso.cruge_fieldvalue where value = '" . $_POST['cedula'] . "'";

            $cedula = Yii::app()->db->createCommand($sql)->query();

            $validarcedula = $cedula->readAll();
            ...
            if (!$validarcedula) {
                if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeStoredUser']])) {
                    ...
                    if ($model->validate()) {
                        ...
                        if (Yii::app()->user->um->save($model, 'insert')) {
                            ...
                            $command->insert('acceso.cruge_authassignment', array(
                                'userid' => $model->iduser,
                                'data' => 'N;',
                                'itemname' => $_POST['rol_name'],
                            ));

                            $command->insert('acceso.cruge_fieldvalue', array(
                                'iduser' => $model->iduser,
                                'idfield' => 0,
                                'value' => $_POST['cedula'],
                            ));
                            ...
                            if (isset($_POST['institucion']) && !empty($_POST['direccion']) && !empty($_POST['coordinacion']) && !empty($_POST['cargo'])) {
                                $command->insert('acceso.cruge_userdetail', array(
                                    'iduser' => $model->iduser,
                                    'institution' => $_POST['institucion'],
                                    'businessoffice' => $_POST['direccion'],
                                    'coordination' => $_POST['coordinacion'],
                                    'appointment' => $_POST['cargo'],
                                ));
                            } else if (isset($_POST['institucion']) && !empty($_POST['direccion']) && !empty($_POST['coordinacion']) && empty($_POST['cargo'])) {
                                $command->insert('acceso.cruge_userdetail', array(
                                    'iduser' => $model->iduser,
                                    'institution' => $_POST['institucion'],
                                    'businessoffice' => $_POST['direccion'],
                            ...
                            $destinatario = $_POST['CrugeStoredUser']['email'];
                            $usuario = $_POST['CrugeStoredUser']['username'];
                            $clave = $_POST['CrugeStoredUser']['newPassword'];
                            $nombres = $_POST['nombre'] . " " . $_POST['apellido'];
                            $cedula = $_POST['cedula'];

                            $envio = Nucleo::envio_correo_usuario($destinatario, 'Sistema Integrado del Poder Popular - SIPP - Compromiso de Confidencialidad', 'Sistema Integrado del Poder Popular - SIPP - Compromiso de Confidencialidad', 'Agradecemos tomar las consideraciones necesarias en cuanto a los puntos que se describen a continuación:<br><br>

                                                                                Deberá por obligación en compromiso de confidencialidad mantener, respetar y resguardar la información manejada dentro del Sistema.<br>
                                                                                El usuario y contraseña son intransferibles de uso reservado, privado y personal.<br><br>

                                                                                Al recibir esta información usted está en la obligación de mantener en secreto y confidencialidad tanto su usuario y contraseña como la data dentro del sistema y en el deber de actuar diligentemente en cuanto al uso, tratamiento, y conservación de dicha información; estableciendo que en todos los casos deberá adoptar los mecanismos necesarios que aseguren y garanticen dicho secreto, y se evite la pérdida y el acceso a la información a terceros no autorizados.<br><br>

                                                                                El incumplimiento de dicha obligación de confidencialidad y secreto puede establecer que la sustracción, revelación y mal uso de dicha información puede ser constitutivo de un ilícito de naturaleza penal y puede ser objeto de acciones disciplinarias y reclamación por daños y perjuicios a este Ministerio.<br><br>

                                                                                Es responsabilidad de cada usuario mantener y resguardar su usuario y contraseña así como el uso y/o manipulación de los procedimientos en los sistemas, la Dirección de Desarrollos y Sistemas no se hace responsable de la información contenida en los mismos.<br><br>

                                                                                <center>Dirección de Desarrollos y Sistemas<br><br>
                                                                                Oficina de Sistemas y Tecnología de la Información </center>', $nombres, $cedula, $usuario, $clave);
                ...
                Yii::app()->user->setFlash('error', "La Cédula ya existe!");
                //$this->render("usermanagementcreate", array('model' => $model,'error'=>$error));
            }
        }
        $this->render("usermanagementcreate", array('model' => $model, 'list' => $list));
    }
    ...
    private function onNewUser(ICrugeStoredUser $model, $newPwd = "") {
        ...
        if ($opt == CRUGE_ACTIVATION_OPTION_INMEDIATE) {
            ...
            //Comentado por Miguel para que envie el correo por la función de nucleo declara en el create
            // Yii::app()->crugemailer->sendPasswordTo($model, $newPwd);
        }
    ...
    public function actionDesactivarUsuarios() {
//        $model= new VswUsuario('search');
        $model = VswUsuario::byUsuariosActivos();


        $this->render('usuarioListado', array(
            'model' => $model,
        ));
    }

    public function actionDatosOrganizacion() {

        $TipoProyecto = Yii::app()->request->getParam('TipoProyecto');
        if ($TipoProyecto == 'usuarios') {
            $model = VswUsuario::byUsuariosActivos();
        } else {
            if ($TipoProyecto == '') {
                $TipoProyecto = 0;
            }
            $model = VswUsuario::byUsuariosEstado($TipoProyecto);
        }
        $this->renderPartial('usuarioListado2', array(
            'model' => $model,
        ));
    }

    public function actionStatusUsuario() {
        $idUsuario = Yii::app()->request->getParam('data');
        $check = Yii::app()->request->getParam('tipoOrg');
        if ($check == 1) {
            CrugeStoredUser::model()->updateByPk($idUsuario, array('state' => 1));
        } else if ($check == 2) {
            CrugeStoredUser::model()->updateByPk($idUsuario, array('state' => 2));
            Yii::app()->db
                    ->createCommand("UPDATE acceso.cruge_session SET status = '0' WHERE iduser=:iduser AND status=:status")
                    ->bindValues(array(':iduser' => $idUsuario, ':status' => 1))
                    ->execute();
        }
        //Yii::app()->user->setFlash('success', '<strong>LA IDEA DEL PROYECTO ' . $_POST['ideaProyecto']['nombre'] . '!</strong> Se Registro Sastifactoriamente.');
        echo CJSON::encode(array('status' => '200', 'redirect' => Yii::app()->createUrl('/cruge/ui/DesactivarUsuarios/'), 'usuario' => $idUsuario));
    }
    ...
    public function actionAjaxResendRegistrationEmailDesactivar($id) {
        $newPassword = CrugeUtil::passwordGenerator();
        $model = Yii::app()->user->um->loadUserById($id);
        if ($model != null) {
            Yii::app()->user->um->changePassword($model, $newPassword);
            Yii::app()->user->um->generateAuthenticationKey($model);
            Yii::app()->user->um->save($model);
            Yii::app()->crugemailer->sendRegistrationEmailDesactivar($model, $newPassword);
            echo CrugeTranslator::t("correo enviado");
        } else {
            echo CrugeTranslator::t("usuario no hallado");
        }
    ...
    public function actionAjaxResendRegistrationEmailDesactivar2() {
        ...
        if ($model != null) {
            ...
            Yii::app()->user->um->save($model);
            //  Yii::app()->crugemailer->sendRegistrationEmailDesactivar($model, $newPassword);
            Nucleo::envio_Usuario_generacion_clave($model, $newPassword);
//             echo CrugeTranslator::t("correo enviado");
            echo CJSON::encode(array('status' => '200'));
        } else {
//            echo CrugeTranslator::t("usuario no hallado");
            echo CJSON::encode(array('status' => '600'));
        }
    }
    ...
    public function actionNoTieneAcceso() {
        $this->render('NoTieneAcceso');
    }

    public function actionLIstDir() {
        $Id = (isset($_POST['inst']['inst']) ? $_POST['inst']['inst'] : $_POST['inst']);
        if (!empty($Id)) {
            $Selected = isset($_POST['inst']) ? $_POST['inst'] : '';
            $criteria = new CDbCriteria;
            $criteria->addCondition('t.parent = :parent');
            $criteria->params = array(':parent' => $Id);
            $criteria->order = 't.parent ASC';
            $data = CHtml::listData(CrugeMain::model()->findAll('parent=:parent', array(':parent' => $Id)), 'idmain', 'detail');
    ...
    public function actionLIstCoor() {
        $Id = (isset($_POST['inst']['dir']) ? $_POST['dir']['dir'] : $_POST['dir']);
        if (!empty($Id)) {
            $Selected = isset($_POST['dir']) ? $_POST['dir'] : '';
            $criteria = new CDbCriteria;
            $criteria->addCondition('t.parent = :parent');
            $criteria->params = array(':parent' => $Id);
            $criteria->order = 't.parent ASC';
    ...
    public function actionLIstCar() {
        $Id = (isset($_POST['inst']['coor']) ? $_POST['coor']['coor'] : $_POST['coor']);
        if (!empty($Id)) {
            $Selected = isset($_POST['coor']) ? $_POST['coor'] : '';
            $criteria = new CDbCriteria;
            $criteria->addCondition('t.parent = :parent');
            $criteria->params = array(':parent' => $Id);
            $criteria->order = 't.parent ASC';
    ...
    public function actionLIstIdUser() {
        if (isset($_POST['id']) && !empty($_POST['id'])) {

            $sql = "select *,(select detail
                from acceso.cruge_main
                where idmain=institution) as inst,(select detail
                from acceso.cruge_main
                where idmain=businessoffice ) as dir,(select detail
                from acceso.cruge_main
                where idmain=coordination) as coor,(select detail
                from acceso.cruge_main
                where idmain=appointment) as car
                from acceso.cruge_userdetail
                where iduser IN (" . $_POST['id'] . ")";
