...
class UiController extends Controller {
    ...
    public function filters() {
        return array_merge(
                ...
                //	SI rbacSetupEnabled es TRUE entonces:
                    //
                //		1. 	el filtro "crearÃ¡" si es necesario la operacion en base
    ...
    public function actionLogin() {
        ...
        if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeLogon']])) {
            ...
            $user_log = $model->username;
            $user_login = str_replace(' ', '', $user_log);
//              var_dump($user_login); die;
        ...
        $this->render('login', array('model' => $model, 'erros' => '3'));
                    Yii::app()->end();
    }
                $idPersonal = Personal::model()->findByAttributes(array('cedula' => $model->username));
                
//                echo '<pre>';var_dump($idPersonal);die;
                if (empty($idPersonal)) {
                    Yii::log(__CLASS__ . "\nCrugeUser->validate es false\n" . CHtml::errorSummary($model), "error");
                } else {
                    $validarCuenta = Trabajador::model()->findByAttributes(array('id_personal' => $idPersonal->id_personal, 'estatus' => 'A'));
                    
                  //  var_dump($user_login); die;
                    
                    
                //   echo "<pre>";var_dump($validarCuenta);
                    if (!empty($validarCuenta->cuenta_nomina)) {
                        $cuenta = substr($validarCuenta->cuenta_nomina, -6);
                        if ($model->password == $cuenta) {
                            // busca estrictamente por su username, a diferencia de loadUser() quien busca por email O username
                            $usuarioFInd = Yii::app()->user->um->loadUserByUsername($model->username);
                            if (empty($usuarioFInd)) {
                                $values = array(
                                    'username' => $model->username,
                                    //'password' => $model->password,
                                    'email' => $model->username . '@minmujer.gob.ve',
                                );
                                ...
                                $authitemName = 'usuario_general';
                                $userId = $usuario->iduser;
                                $rbac->assign($authitemName, $userId);
                            }
                        }
                    } else {
                        if ($validarCuenta->cod_tipo_personal=='06'){
                     ...
                     //        echo "AQUI QUEDO"; die;
                        ...
                        $this->render('login', array('model' => $model, 'error' => '1'));
                        Yii::app()->end();
                        }
                    }
                }
          ...
          // echo "<pre>";  var_dump($model); die;
            if ($model->validate()) {
             ...
             //   echo "AQUI QUEDO2"; die;
                if ($model->login(false) == true) {
                    Yii::app()->getSession()->add('CedulaUser', $model->username);
    ...
    public function actionUserManagementUpdate($id) {
     ...
     public function actionUserManagementUpdate_1($id) {
      
        $this->_editUserProfile_1(Yii::app()->user->um->loadUserById($id), true);
    }
    
    public function _editUserProfile_1(ICrugeStoredUser $model, $boolIsUserManagement) {
    ...
    public function actionUserManagementCreate() {
        ...
        $lis = new VswListarPersonas;

        if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeStoredUser']])) {
            $model->attributes = $_POST[CrugeUtil::config()->postNameMappings['CrugeStoredUser']];
		var_dump($_POST['id_persona']);die;
            ...
            if ($model->validate()) {
                ...
                if (Yii::app()->user->um->save($model, 'insert')) {
                            ...
                            $command->insert('cruge_authassignment', array(
                                'userid' => $model->iduser,
                                'data' => 'N;',
        ...
        $this->render("usermanagementcreate", array('model' => $model, 'lis' => $lis));
    }
...
//        $this->layout = CrugeUtil::config()->registrationLayout;
    ...
    private function _fieldAdminForm($model) {
        ...
        if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeField']])) {
            ...
            //MODIFICANDO INSERT DEL IDFIEL PARA CAMPOS PERSONALIZADO
            $sql="select nextval('cruge_field_idfield_seq');"; 
...
//var_dump($allUsersDataProvider); die;
//  echo'<pre>';
//    die();
        $this->render(
